...
if __name__ == '__main__':
    ...
    time_train, x_train, time_valid, x_valid = train_val_split(G.TIME, G.SERIES, G.SPLITE_TIME)

    # build model
    model_tune = build_model_tuned()

    # Set the learning rate sheduler
    #lr_scheduler = tf.keras.callbacks.LearningRateScheduler(lambda epoch: 1e-8 * 100**(epoch/20))
    #optomizer = tf.keras.optimizers.SGD(momentum=0.9)

    #set the training parameters
    #model_tune.compile(loss=tf.keras.losses.Huber(), optimizer=optomizer)
