...
def plot_loss_and_mae(history):
    # Get the loss and the mae from history logs
    loss = history.history['loss']
    mae = history.history['mae']
    #Get number of pochs
    epochs = range(len(loss))
    #Plot mae and loss
    plot_series(x=epochs, y=(loss, mae), title='Loss and MAE', xlabel='MAE', ylabel='Loss', legend=['Loss', 'MAE'])

    # Only plot the las 80% of the epochs
    zoom_split = int(epochs[-1]*0.2)
    epochs_zoom = epochs[zoom_split:]
    mae_zoom = mae[zoom_split:]
    loss_zoom = loss[zoom_split:]
    plot_series(x=epochs_zoom, y=(loss_zoom, mae_zoom), title='Loss and MAE', xlabel='MAE', ylabel='Loss', legend=['Loss', 'MAE'])
